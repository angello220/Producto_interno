...
// PARÁMETROS DEL PRODUCTO PUNTO	
	localparam N=100;        // Tamaño de los dos vectores A y B que se procesarán
	localparam n=24;         // Define el ancho en bits de cada elemento de A y B 
...
// VARIABLES PARA EL MÓDULO TRIGGER
   logic trigger;	
	...
	Proceso #(.N(N), .IW(IW), .BPC(BPC), .NBY(NBY))
	Proceso(
		.clk(clk),
		.trigger(trigger),
		.mrx_ready(mrx_ready),
