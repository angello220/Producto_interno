// Módulo Adder Tree. Referencia en https://opencores.org/ - Versión para mazimizar throughput
module Atree_throughput	
	#(parameter 
		N = 1, 
		IW = 1, 
	...
	generate
		...
		if (N == 2)
			add_throughput #(.AW(IW), .BW(IW), .OW(OW))
			add_throughput(
				.clk(clk), 
				.a(data[0]), 
				.b(data[1]), 
		...
		// entonces de llega el caso anterior, y finalmente se obtiene un resultado.
			...
			begin
				...
				pairs_throughput #(.N(N), .IW(IW), .OW(OEW))
				pairs_throughput(
					.clk(clk),  
					.data(data), 
					.res(t_res)
...
module pairs_throughput #(parameter N = 1, IW = 1, OW = IW + 1, RS = (N % 2 == 0) ? N/2 : N/2 + 1)(
	input clk,
	...
	endgenerate
endmodule :pairs_throughput

// Módulo encargado de sumar dos números res <= a + b;
module add_throughput #( parameter AW = 1, BW = 1, OW = (AW > BW) ? AW + 1 : BW + 1)(
	input logic clk,
